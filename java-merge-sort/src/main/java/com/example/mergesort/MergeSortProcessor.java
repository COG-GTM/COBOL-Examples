...
public class MergeSortProcessor {
    ...
    private void createTestData() throws IOException {
        ...
        FixedWidthFileWriter.writeFile("test-file-1.txt", testFile1Records);
        ...
        FixedWidthFileWriter.writeFile("test-file-2.txt", testFile2Records);
    }
    
    private void mergeFiles() throws IOException {
        List<CustomerRecord> file1Records = FixedWidthFileReader.readFile("test-file-1.txt");
        List<CustomerRecord> file2Records = FixedWidthFileReader.readFile("test-file-2.txt");
        
        List<CustomerRecord> mergedRecords = new ArrayList<>();
        mergedRecords.addAll(file1Records);
        mergedRecords.addAll(file2Records);
        
        Collections.sort(mergedRecords, Comparator.comparingInt(CustomerRecord::getCustomerIDAsInt));
        
        FixedWidthFileWriter.writeFile("merge-output.txt", mergedRecords);
    ...
    private void sortByContractID() throws IOException {
        List<CustomerRecord> mergedRecords = FixedWidthFileReader.readFile("merge-output.txt");
        
        Collections.sort(mergedRecords, Comparator.comparingInt(CustomerRecord::getContractIDAsInt).reversed());
        
        FixedWidthFileWriter.writeFile("sorted-contract-id.txt", mergedRecords);
